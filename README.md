...
Run over the whole corpus on the card, 287 of the 303 comparable kernels come out computing byte-identical results to the vendor schedule. The ones that do not are named in [findings](docs/FINDINGS.md) rather than summarised into a percentage, because a limitation with a name gets fixed.
...
> [!NOTE]
> **Alpha, and specific about what that means.** What is done: both oracles, the instruction database with its fields proven writable, the hazard checker over a real control-flow graph, latency measured on one SKU by three independent methods, and a scheduler that round-trips 287 of 303 comparable corpus kernels through the hardware byte-for-byte. What is not: 16 kernels it still schedules wrong, named in the findings rather than hidden; many opcodes still carry assumed latencies rather than measured ones; and only one GPU has been measured. Where something is inferred rather than measured, the tooling says so rather than rounding it up to a fact. See the [roadmap](docs/ROADMAP.md) and the [method](docs/METHOD.md).
