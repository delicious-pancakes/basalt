...
| :--- | :--- | :--- | ---: |
| **Assembler** | SASS text to the 128-bit word | Reassemble every instruction `ptxas` emitted and compare bytes | 11,743 of 11,752 exact, **0 wrong** |
| **Checker** | Reads a schedule, reports hazards | The vendor's own output must verify clean, and a deliberately shortened stall must be caught | 0 errors on 423 vendor kernels, **0 missed** on 223 broken ones |
| **Scheduler** | Assigns the control bits from scratch | Discard every control bit, compute new ones, run both on the GPU against four inputs, compare output bytes | **417 of 417** byte-identical, at every optimisation level |

And the part a scheduler is usually quiet about: what the correctness costs. basalt's
schedules spend **0.93x** the vendor's issue cycles, slower on 44 of the 436 kernels and
...
```bash
python scripts/roundtrip_corpus.py    # reschedule all 436 corpus kernels, run both on the GPU
```
...
**And when it says a schedule is unsafe, the silicon agrees.** Take the vendor's own working schedule for 223 kernels, shorten one real dependency in each, and compare basalt's verdict against what the GPU computes: 73 that it called broken were broken, and **nothing it called safe computed a wrong answer**. That number started at 34 missed rather than zero, and [findings](docs/FINDINGS.md) says what the cause was and what fixing it cost in false alarms, because a sweep that only ever reported its final figure would be worth less than one that reported its first.
