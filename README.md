...
Run over the whole corpus on the card, 275 of the 303 comparable kernels come out computing byte-identical results to the vendor schedule. The ones that do not are named in [findings](docs/FINDINGS.md) rather than summarised into a percentage, because a limitation with a name gets fixed.
