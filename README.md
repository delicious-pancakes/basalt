...
Six of seven test kernels come out computing exactly what the vendor schedule computes. The one that does not is recorded as a strict expected failure with its reason rather than removed, because a limitation that still runs is one that reports the day it is fixed.

That loop is where the real bugs came from. Stall spent outside the window between a producer and its consumer counts for nothing, and spending it there ends the search with a program that is still short. A stall pinned to the safe encoding was being overwritten by a later pass, replacing a guarantee with a small number. fp64 operands occupy register pairs with nothing in the mnemonic to say so, so half of every fp64 dependency was invisible to both the checker and the scheduler. And a predicate used as an instruction's guard needs thirteen cycles where the same predicate read as data needs five, because a guard has to be resolved before the instruction issues at all. None of those were found by reasoning; every one was found by running the output and getting the wrong number.

> [!NOTE]
> **Alpha, and specific about what that means.** What is done: both oracles, the instruction database with its fields proven writable, the hazard checker over a real control-flow graph, latency measured on one SKU by three independent methods, and a scheduler that round-trips six of seven test kernels through the hardware. What is not: the scheduler shares a blind spot with the verifier on fp64, recorded as an expected failure; many opcodes still carry assumed latencies rather than measured ones; and only one GPU has been measured. Where something is inferred rather than measured, the tooling says so rather than rounding it up to a fact. See the [roadmap](docs/ROADMAP.md) and the [method](docs/METHOD.md).
